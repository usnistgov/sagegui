...
- **Next action (next session):** Three UI items are planned and sequenced, ahead of templates and theme. Do them in order: (1) **surface the prefilter options** (`prefilter`, `prefilter_chunk_size`, `prefilter_low_memory`) on Files & Database, so semi-enzymatic and non-specific searches do not exhaust memory — includes rewriting the `docs/PARAMETER_REFERENCE.md` section, whose stated `prefilter_low_memory` default is wrong (see NOTES → Database prefiltering); (2) **remember settings between sessions** via eframe's `persistence` feature — closing the window is the current way to abort a run, and it loses every parameter; (3) **Stop button** that cancels at step boundaries. A fourth item — a fork patch to interrupt a search already in progress — is optional, and should be judged after (2) lands. Still open after these: Load Config round-trip with `fasta_paths`, JSON-file templates, and the high-contrast flat light theme (NOTES UI-review #7).
...
- [ ] **Remember settings between sessions** — Persist `Config` and the tolerance-type selections through eframe's `persistence` feature (not a default feature; enable it in `Cargo.toml`). Restore at startup, save on exit and on eframe's auto-save interval. This is the practical fix for aborting a run: closing the window already cancels the search correctly, but today it also discards every parameter. **Not blocked by the deferred Save/Load Config work** — that was blocked on matching Sage's `results.json` schema, and persisting our own `Config` to our own file needs no schema alignment. Every new `Config` field must carry `#[serde(default)]` so a stored blob survives the addition. While here: closing the window mid-run leaks the temp concatenated FASTA, because `cleanup_thread` never runs.
- [ ] **Stop button** — Cancel a run from the run bar. A GUI-only version can cancel at step boundaries (a stop asked for during the database build takes effect when the build ends). Interrupting a search that is already scoring spectra needs a cooperative-cancellation flag in `neely/sage`, in the same additive shape as the `Runner.progress` patch — optional, and worth judging only after settings persistence lands. Verified: Sage writes every output file at the end of `run()`, so a cancelled run leaves nothing behind.
...
- [ ] **Prefilter options (for large databases)** — Surface `prefilter`, `prefilter_chunk_size` and `prefilter_low_memory` on Files & Database, between the FASTA list and the Advanced block. The goal is memory headroom for semi-enzymatic and non-specific searches. Default all three to what Sage resolves to on its own (`false` / `0` / `true`), so a SageGUI run matches a Sage CLI run with the same visible config. Two things to handle at the same time: the `docs/PARAMETER_REFERENCE.md` section states the wrong `prefilter_low_memory` default and needs a rewrite, and enabling the checkbox makes a phase reachable that reports no progress at all (see NOTES → Database prefiltering).
