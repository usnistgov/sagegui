...
- **Current phase:** Phase 5 in progress. Licensing resolved (Apache-2.0, LICENSE file added 2026-08-16). Async run-bar progress done 2026-08-21. Prefilter controls, settings persistence, a step-boundary Stop button, and a live Sage-log panel landed 2026-08-21 (see below). Templates and theme still open.
- **Last updated:** 2026-08-21
- **Next action (next session):** Several items landed this session — prefilter controls, settings persistence, Stop button, and now a live "Sage Log" panel on Run/Info (captures Sage's own log output via a wrapped logger, no fork changes) — `cargo build`/`clippy --all-targets`/`fmt`/`test` all clean throughout. **None of it has been verified live in the GUI yet** — no native-macOS automation tool was available this session, only browser/iOS-Simulator. First action next session (or the maintainer, interactively — baseline files are at `~/Documents/proteomicsTesting/`, see NOTES "Test baseline"): (1) re-run the Phase 2 baseline with prefiltering off, confirm still 60,672 PSMs; (2) re-run with prefiltering on, confirm a plausible different count; (3) close/reopen the app after changing settings, confirm they restore; (4) click Stop mid-run, confirm the "stops at end of current step" behavior and the temp-FASTA cleanup on window close; (5) confirm the Sage Log panel actually populates and auto-scrolls during a run. After that: a fork patch to let Stop interrupt an in-progress search is optional (design sketched in NOTES, deliberately not built — judge whether it's still needed once persistence has been used a while). Also still open: Load Config round-trip with `fasta_paths`, JSON-file templates, the high-contrast flat light theme (NOTES UI-review #7), and re-homing the Info/Help block off the Run screen (NOTES UI-review #3).
...
- [x] **Remember settings between sessions** — `Config`, the tolerance-type selections, experiment archetype, and active tab now persist through eframe's `persistence` feature (enabled in `Cargo.toml`; not a default feature). Restored at startup, saved on exit and on eframe's 30-second auto-save interval. This is the practical fix for aborting a run: closing the window already cancels the search correctly, but previously also discarded every parameter. Not blocked by the deferred Save/Load Config work — that was blocked on matching Sage's `results.json` schema, and persisting our own `Config` to our own file needs no schema alignment. Every `Config` field carries `#[serde(default)]` so a stored blob survives future field additions. Closing the window mid-run no longer leaks the temp concatenated FASTA — cleaned up in `on_exit`. *(Landed 2026-08-21.)*
- [x] **Stop button** — Cancels a run from the run bar. Cancels at step boundaries: a stop asked for during the database build or prefilter pass takes effect when that phase ends. A search already scoring spectra is **not yet interrupted mid-flight** — the button is honestly labelled ("stops at the end of the current step"), and status reports "Search stopped. No output files were written." Interrupting an in-progress search needs a cooperative-cancellation flag in `neely/sage`, in the same additive shape as the `Runner.progress` patch — deferred; see NOTES for the design. Verified: Sage writes every output file at the end of `run()`, so a cancelled run leaves nothing behind. *(Landed 2026-08-21; fork-patch follow-up not started.)*
