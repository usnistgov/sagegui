...
- **Current phase:** Phase 5 largely done. `v0.9.0` is the latest release (2026-09-22): Convert results (mzIdentML 1.1.1 and pepXML 1.23) with its own Results location on the Run / Info tab, a Write HTML report checkbox, a pre-run check of the output folder, and a Combine Charge States hover note. Run / Info now has two groups: Search output (what Sage writes) and Results (its own Results location, then Convert). The maintainer ran both conversions and all three Search output checkboxes in the app before the cut: Convert worked by hand and by auto-run-after-search, the Results location follow rule worked, the softer success colours read correctly on the light theme, and `results.sage.pin`, `results.sage.report.html` and `matched_fragments.sage.tsv` were all written. Only `results.sage.report.html` was opened and read; the other two were seen to exist but not inspected. CI failed twice on `main` before the cut: Windows's `xmllint` refuses to compile the upstream pepXML schema (a real Unique Particle Attribution defect in that schema, not our output). Fixed by treating a schema-compile failure as a skip, not a test failure (commit `b4adbbf`); confirmed green on all four platforms before tagging. After the cut (2026-09-22), `results.sage.pin` and `matched_fragments.sage.tsv` from the v0.9.0 test run were inspected in full for the first time (scripted checks against `results.sage.tsv`: zero orphan references, zero mismatches, a hand-verified fragment count on one PSM) and both mzIdentML and pepXML were parsed by an independent reader ([pyteomics](https://pyteomics.readthedocs.io/) + `psims`) with zero errors on all 9,392 entries each, plus a real rescoring run through [mokapot](https://mokapot.readthedocs.io/) on the PIN file. Details in NOTES → "Independent-reader verification". No target tool (Scaffold, Skyline, PeptideShaker, TPP) has opened either file yet. Previous: `v0.8.2` fixed macOS downloads that would not open ("damaged and can't be opened"). The block is Gatekeeper refusing an app Apple has not notarized; by maintainer decision there is no Apple Developer account for now, so the README gives the `xattr` command instead. Before that, `v0.8.1` stopped the app deleting its own error messages and added a check for selected files that no longer exist.
- **Last updated:** 2026-09-22
- **Next action (next session):** Open the converter output in an actual target tool (Scaffold, Skyline, PeptideShaker or TPP) — an independent parser and a real rescoring tool have now confirmed the files are well-formed and semantically sane (see NOTES), but nothing has confirmed a specific downstream importer accepts them. Three older things are still unchecked. **Windows SmartScreen:** the Windows `.exe` is also unsigned, and nobody has checked what a browser download shows there. **The Intel GUI app:** the x86_64 bundle passes strict verification, but it has not been launched on Intel hardware or under Rosetta. **The dialog text after sealing:** tested that a sealed, quarantined bundle still does not launch, but not whether macOS still calls it "damaged". Then the **SageGUI versus Sage defaults audit**, as an invariant over an empty Sage config. The licence decision and the rename checklist are scoped and waiting.
