...
- **Current phase:** Phase 5 in progress. Licensing resolved (Apache-2.0, LICENSE file added 2026-08-16). Async run-bar progress done 2026-08-21. Prefilter controls, settings persistence, and a step-boundary Stop button landed 2026-08-21 (see below). Templates and theme still open.
- **Last updated:** 2026-08-21
- **Next action (next session):** The three planned UI items (prefilter controls, settings persistence, Stop button) are implemented — `cargo build`/`clippy`/`fmt`/`test` all clean, three new unit tests cover the prefilter serde-migration path. **Not yet verified live in the GUI** — no native-macOS automation tool was available this session, only browser/iOS-Simulator. First action next session (or the maintainer, interactively — baseline files are at `~/Documents/proteomicsTesting/`, see NOTES "Test baseline"): (1) re-run the Phase 2 baseline with prefiltering off, confirm still 60,672 PSMs; (2) re-run with prefiltering on, confirm a plausible different count; (3) close/reopen the app after changing settings, confirm they restore; (4) click Stop mid-run, confirm the "stops at end of current step" behavior and the temp-FASTA cleanup on window close. After that: a fork patch to let Stop interrupt an in-progress search is optional (design sketched in NOTES, deliberately not built — judge whether it's still needed once persistence has been used a while). Also still open: Load Config round-trip with `fasta_paths`, JSON-file templates, and the high-contrast flat light theme (NOTES UI-review #7).
