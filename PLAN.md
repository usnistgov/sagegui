...
- **Current phase:** Phase 5 in progress. Licensing resolved (Apache-2.0, LICENSE file added 2026-08-16). Async run-bar progress, prefilter controls, settings persistence, a Stop button, and a live Sage-log panel all landed 2026-08-21 — live-tested for the first time 2026-08-24, which found real bugs in two of them (see below). Templates and theme still open.
- **Last updated:** 2026-08-24
- **Next action (next session):** First live test of the 2026-08-21 features happened 2026-08-24 (Mac). Results: progress bar and Sage Log panel confirmed **working** (including the expected quiet period during database build — see NOTES); prefilter run completed (output at `~/Documents/proteomicsTesting/delme/blah`, compare against the non-prefiltered baseline in `delme` when convenient). Two real bugs found, both reopened in the checklist below with full detail in NOTES: **(1) Stop button** — during an active search it doesn't stop (expected) but leaves the run bar stuck and falsely claims no output was written when the run actually completed normally; don't trust it until fixed. **(2) Settings persistence** — modifications don't survive a restart, and it's unconfirmed what else might not; needs a full field-by-field audit, not just a mods patch. Smaller items also logged: a macOS-only terminal window opens alongside the GUI (Windows already fixed), the green "Processing" run-bar label reads poorly, and the app icon needs real branding. **Not yet tested on Windows at all** — Mac-only so far. Start next session with the Stop-button bug (highest-impact: users can't trust it not to silently discard or silently keep output). Also still open: Load Config round-trip with `fasta_paths`, JSON-file templates, the high-contrast flat light theme (NOTES UI-review #7), and re-homing the Info/Help block off the Run screen (NOTES UI-review #3).
...
- [ ] **Stop button** — Cancels a run from the run bar. *(Landed 2026-08-21 for the pre-search phase — confirmed working. 2026-08-24 live test: clicking Stop **during** an active search does not stop it (expected, by design) but leaves the run bar in a confusing stuck state and reports "No output files were written" even though the run actually completed and wrote output normally — see NOTES "Stop button" for the exact mechanism. Reopened; do not trust the Stop button or its message until this is fixed.)*
