...
- **Current phase:** Phase 5 in progress. Licensing resolved (Apache-2.0, LICENSE file added 2026-08-16). Async run-bar progress, prefilter controls, settings persistence, a Stop button, and a live Sage-log panel all landed 2026-08-21 — live-tested for the first time 2026-08-24, which found real bugs. The Stop-button false-message bug is fixed and live-tested. Real mid-search cancellation (a `neely/sage` fork patch, commit `ed5f06c`) was then built the same day at the maintainer's explicit request, after live testing showed the message fix alone wasn't enough — **this part is code-complete but NOT yet live-tested.** Settings-persistence audit and templates/theme still open.
- **Last updated:** 2026-08-24
- **Next action (next session):** **Live-test real Stop-button cancellation before anything else.** `neely/sage` (`ed5f06c`) now has a cooperative `Runner.cancel` flag checked inside the scoring loop, between file chunks, and before FDR/grouping/quant/write — `sagegui`'s Stop button feeds it the same flag already used for pre-search checkpoints. Compiles clean, unit-tested at the message-handling level, but **no live GUI run has confirmed clicking Stop mid-scoring now actually shortens the run and writes nothing** — see NOTES "Stop button" for the exact reproduction (click Stop partway into a real search's scoring phase, confirm early exit + no new output files). Do not mark this done until that passes. Two small new UX gaps in the Sage Log panel also surfaced in testing (not urgent, see NOTES "Stop button" → new follow-ups): log text isn't selectable/copyable, and the panel briefly looked empty before filling in. After the Stop live-test: **(2) Settings persistence** — modifications don't survive a restart, and it's unconfirmed what else might not; needs a full field-by-field audit, not just a mods patch. Smaller items also logged: a macOS-only terminal window opens alongside the GUI (Windows already fixed), the green "Processing" run-bar label reads poorly, the app icon needs real branding, and the two Sage Log UX gaps above. **Not yet tested on Windows at all** — Mac-only so far. Also still open: Load Config round-trip with `fasta_paths`, JSON-file templates, the high-contrast flat light theme (NOTES UI-review #7), and re-homing the Info/Help block off the Run screen (NOTES UI-review #3).
...
- [ ] **Stop button** — Cancels a run from the run bar. *(Landed 2026-08-21 for the pre-search phase — confirmed working. 2026-08-24 live test found a real bug: clicking Stop **during** an active search didn't stop it and falsely reported "No output files were written" even though the run completed and wrote output normally. Message bug fixed and live-tested same day. That test also surfaced that non-interruption itself was unacceptable, not just the message — so real cooperative cancellation was built the same day via a `neely/sage` fork patch (commit `ed5f06c`, see NOTES "Stop button" and "Custom patch carried on neely/sage: Runner.cancel"). **Not yet closed** — the cancellation patch compiles and passes unit tests but has not been live-tested; needs a click-Stop-mid-scoring reproduction confirming the run actually shortens and writes no output before this checkbox ticks.)*
