...
- **Current phase:** Phase 5 largely done. `v0.8.2` is the latest release (2026-09-10). Since then, unreleased on `main` (2026-09-21): a Write HTML report checkbox, a pre-run check of the output folder, a Combine Charge States hover note, and Convert results (mzIdentML 1.1.1 and pepXML 1.23) on the Run / Info tab. No version is cut yet. `v0.8.2` fixes macOS downloads that would not open ("damaged and can't be opened"). The block is Gatekeeper refusing an app Apple has not notarized; by maintainer decision there is no Apple Developer account for now, so the README gives the `xattr` command instead. The same investigation found the macOS bundle failed Apple's own strict signature check (arm64 bundle never sealed, x86_64 binary unsigned). CI now seals the bundle and verifies the archive users download, and the published v0.8.2 zips were checked by hand: both architectures pass strict verification, and the arm64 app launches once quarantine is removed. Before that, `v0.8.1` stopped the app deleting its own error messages and added a check for selected files that no longer exist.
- **Last updated:** 2026-09-21
- **Next action (next session):** First, **look at the new Convert results group in the running app.** Nobody has seen it by eye: layout, button states, progress bar and status colours are covered only by unit tests. Then run the two output files through a real reader (Skyline, PeptideShaker, TPP or Scaffold), because `xmllint` does not check CV terms. Then decide whether to cut `v0.9.0`. Three older things are still unchecked. **Windows SmartScreen:** the Windows `.exe` is also unsigned, and nobody has checked what a browser download shows there. **The Intel GUI app:** v0.8.2's x86_64 bundle passes strict verification, but it has not been launched on Intel hardware or under Rosetta. **The dialog text after sealing:** tested that a sealed, quarantined bundle still does not launch, but not whether macOS still calls it "damaged". Then the **SageGUI versus Sage defaults audit**, as an invariant over an empty Sage config. (The stale-reference check on the persisted output directory is done: commit de1452b.) The licence decision and the rename checklist are scoped and waiting.
- **Released:** `v0.8.2` (2026-09-10) — macOS: seals the app bundle, verifies the downloaded archive in CI, and corrects the README's Gatekeeper instructions ("damaged", the `xattr` command, and how to recover from Move to Trash). Previous: `v0.8.1` — a failed run now shows its error, and missing files are caught before the run. `v0.8.0` — experiment templates, Sage config/results import, enzyme presets, delta-mass tolerance display, NIST FAIR governance files, and the macOS Intel binary that was actually Apple Silicon.
...
- [ ] **pepXML / mzIdentML export** — **decided 2026-09-21: build both inside SageGUI** (not psm-utils, not shic). Reasons are in NOTES → "Built here: mzIdentML 1.1.1 and pepXML 1.23 converters". **Status: converter and UI done, unreleased (2026-09-21).** The Convert results group is on the Run / Info tab: two buttons, a q-value source and limit, a decoys switch, and two run-after-search checkboxes. It runs on its own thread with progress and Cancel. `src/export/` turns `results.sage.tsv` and `results.json` into `results.sage.mzid` and `results.sage.pep.xml`. mzIdentML 1.1.1 passes `xmllint` against the published XSD on the fixture and on both full real outputs. pepXML 1.23 passes except for one attribute: `search_engine="Sage"` is not in the schema's list (deliberate, see NOTES). **Still to do:** a look at the UI in the running app, and a test with a real reader (Scaffold, Skyline, PeptideShaker or TPP), which nobody has done. Open questions: whether readers accept `MS:1001412` and `MS:1001413` written in delta-mass terms (see NOTES), and multi-file and multi-rank runs, which only hand-made rows cover.
