...
- **Current phase:** Phase 5 largely done. `v0.8.1` released 2026-09-10, a fix release for two problems found running v0.8.0 on Windows with eight files. The first was self-inflicted: v0.8.0's rule for clearing a settings error matched on message text, and a finished run's failure has the same shape, so every real run failure was erased one frame after it appeared. The Run button flashed and said nothing. v0.8.0 had also added `catch_unwind` so a panic reports instead of hanging, and that report was wiped by the same rule; both shipped together and cancelled out. The second was the underlying cause, visible once messages survived: a FASTA path whose file was no longer there. Pre-flight now checks that every selected file exists, which persistence makes necessary.
- **Last updated:** 2026-09-10
- **Next action (next session):** The obvious audit, from the last debrief and now reinforced: **SageGUI's defaults versus Sage's**. Three disagreements have surfaced by accident rather than by looking (`restrict` absent means no restriction, `isotope_errors` absent means `(0,0)`, and `EnzymeParameters` defaults `missed_cleavages` to 1 where ours is 2). Assert that applying an empty Sage config leaves `Config` equal to what Sage itself resolves, rather than checking fields one at a time. Then the cheap items: a hover note on Combine Charge States (our default of true makes MSstats' `PrecursorCharge` meaningless), session resilience, the results-summary panel, and a smarter default output directory. The **licence decision** and the **rename checklist** are both scoped and waiting.
- **Released:** `v0.8.1` (2026-09-10) — fixes a failed run reporting nothing at all, and catches selected files that no longer exist before the run starts. Previous: `v0.8.0` — experiment templates, Sage config/results import, enzyme presets, delta-mass tolerance display, NIST FAIR governance files, and the macOS Intel binary that was actually Apple Silicon. `v0.7.1` — real Stop-button cancellation, settings persistence, app icon, macOS `.app` bundle.
