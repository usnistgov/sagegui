...
- [ ] **Reset-to-defaults on numeric controls** — maintainer request 2026-09-09, from live testing: "in case I hit the slider and forget what it should be at". Wanted on Missed Cleavages, Min/Max Length, the peptide mass range, and the sliders under Advanced. Applying a template already resets everything, so this is the finer-grained version for one control at a time. Options to weigh: a small reset affordance per control, one "reset this section" button per group, or showing the default value in the hover text so the number can be typed back by hand. The last is nearly free and may be enough.
