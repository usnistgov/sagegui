...
- **Stop button** — cancels a run from the run bar, but only if clicked before the search itself starts (database build/prefilter phase); a search already scoring spectra is not interrupted and runs to completion. **Known issue (found in live testing 2026-08-24):** clicking Stop during an in-progress search does not stop it, and the run bar can end up in a stuck "Stopping" state — needs investigation before this is reliable. Not yet safe to rely on.
