...
- **Stop button** — cancels a run from the run bar. Initially landed 2026-08-21 covering only the pre-search phase; extended 2026-08-24 with real cooperative cancellation of an in-progress search too (a `neely/sage` fork patch — see Fixed below and NOTES.md). Not an instant kill: a parallel scoring batch already in flight finishes its cheap remainder, and the FASTA-digest phase still can't be shortened once started. **Not yet live-tested against a real in-progress cancellation** — verify before relying on it.
