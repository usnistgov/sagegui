...
### Fixed
- **The macOS Intel download was an Apple Silicon binary** — both macOS builds ran on the same Apple Silicon runner and neither passed `--target`, so `sage-launcher-macos-x64.zip` and `sage-launcher-macos-arm64.zip` were byte-identical and both contained an `arm64` binary. Anyone on an Intel Mac downloaded something that could not run. This had been true since v0.7.1. Each architecture now builds on its own native runner.
