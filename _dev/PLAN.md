...
- **Current phase:** Publication as `usnistgov/sagegui`, in progress (2026-09-23). Done on `main`: Sage vendored in `vendor/sage` (the build no longer uses `neely/sage`), the optional database cache, licence files in the usnistgov/sageRecon shape, the dev record moved to `_dev/`, and CI narrowed to tags and manual runs. See JOURNAL 2026-09-23. Phase 5 otherwise largely done; `v0.9.0` (2026-09-22) is the latest release.
- **Last updated:** 2026-09-23
- **Next action (next session):** Finish the publication (plan file `~/.claude/plans/i-think-i-am-declarative-mochi.md`): rewrite `e6ccd69..main` to the NIST author address without AI trailers, push to `usnistgov/sagegui` with Ben's approval, recreate releases v0.6.0 to v0.9.0 from the original `neely/sagegui` assets, then repoint URLs, clean up the Info / Help group on Run / Info (one coherent block: fork statement, maintainer, the usnistgov repository for issues, licence pointer, how to cite SageGUI and Sage), enable Actions and branch protection, and archive `neely/sagegui`. After that: a first manual CI run to prove the Windows build of vendored Sage and the cache, then the older open items (a target tool reading the converter output, Windows SmartScreen, the Intel GUI app, the SageGUI versus Sage defaults audit).
