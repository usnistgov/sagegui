...
- **Correctness check:** `index_cache::tests::cached_database_gives_the_same_search_results` (ignored; set `SAGEGUI_CACHE_E2E` to a `results.json`). On the serum search, all 32,221 PSMs matched a fresh build in every column except `psm_id`, and sorted `lfq.tsv` was identical.
- **Key (do not weaken):** xxHash3-128 over format version, strategy byte, `SAGE_COMMIT`, the SageGUI version, the canonical JSON of the resolved `Parameters` without `fasta` and the three prefilter keys, and the FASTA content hash. Always hash the content; never trust size and mtime (a stale hit is a wrong result). `serde_json` has no `preserve_order` in this tree, so map order cannot reach the key; if a dependency ever enables `preserve_order`, `cache_key_does_not_depend_on_map_order` fails.
- **Format:** little-endian binary with exact float bits, not JSON (JSON rounds `f32` through decimal text). Accessions and sequences are stored once and shared through `Arc` on load. Mirror code in `write_payload` / `read_payload` names every `IndexedDatabase` and `Peptide` field in a struct literal, so a new Sage field is a compile error at the next re-vendor. Bump `FORMAT_VERSION` for any layout change, or for a Sage patch that changes what the database holds.
- **Failure is a miss (intentional):** a bad file returns `None` and the run builds as usual. The reader catches its own panics, because on Windows (`windows_subsystem = "windows"`) a panic in the search thread becomes a failed run. A read failure never deletes the file; the next store overwrites it.
- **Cache folder (std only, no `directories` crate):** macOS `~/Library/Caches/gov.nist.sagegui/index-cache`, Windows `%LOCALAPPDATA%\SageGUI\index-cache`, Linux `$XDG_CACHE_HOME/sagegui/index-cache` or `~/.cache/sagegui/index-cache`. Adding `directories` re-resolved `windows-sys` for several existing Windows-only crates, which could not be checked with CI off, so it was dropped. 20 GiB in total, least recently used first; a load marks the entry as used.
- **Temp FASTA constraint:** for several FASTAs the key hashes the concatenated temp file on the search thread. `cleanup_thread` and `on_exit` delete it only after the thread ends. Keep that order.
