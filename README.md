...
This fork compiles Sage from source vendored in [`vendor/sage`](vendor/sage/VENDORED.md): upstream `lazear/sage` at commit `d74024d` (10 commits after the v0.15.0-beta.2 release), plus three small additive code patches (search progress, the Stop button and the database cache) and three dependency updates that clear security advisories, all listed in [`PATCHES.md`](vendor/sage/PATCHES.md).
...
- Database cache: save the built peptide database and reuse it on a later run with the same FASTA and database settings. The code is in place but hidden in the UI until it is ready.
...
- [Sage](https://github.com/lazear/sage) is the search engine
...
> search engine* (Version nist-v0.10.0) [Computer software]. National Institute of
...
its tag (for example `nist-v0.10.0`). The Run / Info tab prints the SageGUI version
number (`0.10.0` for `nist-v0.10.0`) and the Sage engine version it was built against.
